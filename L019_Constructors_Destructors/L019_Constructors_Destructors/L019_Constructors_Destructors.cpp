...
// Порегрузка
Auto::Auto(char *auto_model, int date_year, int date_mouth, int date_day)
{
...
int main(int arg, char *argv[])
{
	...
	Auto shkoda ("Shkoda", 2015, 2, 13);	// создать объект "shkoda" с перегоузкой консруктора 
	shkoda.message();						// взять метод
	shkoda.set("Shkoda", 2005, 11, 23);		// передать параметры в объект класса
	shkoda.get();							// получить параметры из объекта класса 	

	Auto bmw;
	bmw.set("BMW", 2012, 1, 11);
	bmw.get();
